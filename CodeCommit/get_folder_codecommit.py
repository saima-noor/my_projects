    ...
    try:
        ...
        while True:
            ...
            for repo in response1['repositories']:
                ...
                url = response2['repositoryMetadata']['cloneUrlHttp']
                print(url)

                elements_url = url.split('/', 4)
                result = {
                'https': elements_url[0],
                'space': elements_url[1],
                'link': elements_url[2],
                'folder': elements_url[4]
                }
                print(result['folder'])
                ...
                response3 = codecommit_client.get_file(repositoryName= repo_name, filePath= "cockpit-discovery/aws-profile-setup.sh" ) 
                print(response3)
